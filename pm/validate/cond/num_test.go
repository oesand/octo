...
func TestMinCondition(t *testing.T) {
	tests := []struct {
		...
		{"value less than min", 5, 3, fmt.Errorf("must be >= 5")},
		{"value equals min (zero)", 0, 0, nil},
		{"value greater than min (zero)", 0, 1, nil},
		{"value less than min (zero)", 0, -1, fmt.Errorf("must be >= 0")},
	}
...
func TestMaxCondition(t *testing.T) {
	tests := []struct {
		...
		{"value greater than max", 10, 15, fmt.Errorf("must be <= 10")},
		{"value equals max (zero)", 0, 0, nil},
		{"value less than max (zero)", 0, -1, nil},
		{"value greater than max (zero)", 0, 1, fmt.Errorf("must be <= 0")},
	}
